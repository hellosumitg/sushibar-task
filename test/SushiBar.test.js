...
describe("SushiBar", function () {
  ...
  it("should not allow enter if not enough approve", async function () {
    await expect(this.bar.enter("100")).to.be.revertedWith(
  ...
  it("should work with more than one participant", async function () {
    ...
    await this.sushi
      .connect(this.bob)
      .approve(this.bar.address, "100", { from: this.bob.address });
    ...
    await this.sushi
      .connect(this.carol)
      .transfer(this.bar.address, "20", { from: this.carol.address });
